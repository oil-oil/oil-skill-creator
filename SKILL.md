...
先读 [Review 与整改规范](references/review-and-remediation.md)。在运行校验器前，先列出需求适用表：每项标记“适用 / 不适用及原因 / 待确认”，再登记实现位置和验证证据。批量评审必须逐个 Skill 登记，不能用总体通过代替。

优先核对：是否调用外部服务、是否需要用户输入 Key、有没有真实可用的现成配置入口。桌面需要单行 Key 且没有现成安全入口时，**必须接入固定配置页**；新增 getpass、环境变量支持或系统存储，均不能代替页面。页面保存后必须用实际业务读取端验收，详见[凭据输入页](references/credential-ui.md)。

Review 同时检查静态缺陷和程序无法判断的产品问题，不把“校验器通过”等同于“Skill 有用”。
...
description 和 README 开头先讲用户能完成的任务、得到的产物和适用边界。不要用模型版本、服务商或当前宿主的名字代替功能介绍；只有决定触发对象或真实兼容范围的专用产品名才保留。服务、模型、环境变量和命令等准确名称放在配置或适配说明，不因品牌中立而隐瞒数据去向。
...
- 整改：需求适用表逐项有实现和验证证据，适用项没有遗漏；凭据配置已核验页面、保存、恢复和实际业务读取，或有可复用现成入口的具体证据。P0/P1 已处理或明确接受，相关回归测试完成，没有覆盖无关内容；只有本次需要前后效果对照时，才要求快照和对照评估完整。
