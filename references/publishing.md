...
仓库 About、README 首段和 Skill description 先讲功能、交付物与适用边界，尽量不堆叠服务商、宿主和模型版本。专用产品名称只在确实影响触发、兼容性或用户选择时保留；配置命令、依赖与数据接收方必须准确，不能为减少品牌提及而隐藏。
...
- 凭据需求适用表有结论；需要桌面 Key 输入且无现成安全入口的 Skill 已交付固定页面，并验证保存、恢复和真实业务读取；
